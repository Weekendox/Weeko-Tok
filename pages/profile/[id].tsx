...
const Profile = ({ data }: IProps) => {
  ...
  return (
    <>
      <Head>
        <title>{user.userName} Profile | Weeko App</title>
      </Head>
      <div className="w-full">
        ...
        <div>
          <div className="flex gap-10 mb-10 mt-10 border-b-2 border-gray-200 bg-white w-full">
            <p
              className={`text-xl font-bold cursor-pointer mt-2 ${videos}`}
              onClick={() => setShowUserVideos(true)}
            >
              Videos
            </p>
            <p
              className={`text-xl font-bold cursor-pointer mt-2 ${likedVideos}`}
